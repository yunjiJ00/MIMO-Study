...
for K in K_values:
    ...
    plt.plot(snr_db, avg_sum_rate_dpc[K], linestyle=linestyle, marker='s')
    plt.plot(snr_db, avg_sum_rate_wsrbf_wmmse1[K], linestyle=linestyle, marker='o')
    plt.plot(snr_db, avg_sum_rate_wsrbf_wmmse2[K], linestyle=linestyle, marker='+')
labels = ['DPC Capacity Bound', 'WSRBF-WMMSE1', 'WSRBF-WMMSE2']
markers = ['s', 'o', '+']
plt.figlegend(labels=[f"Marker '{m}': {l}" for m, l in zip(markers, labels)], 
